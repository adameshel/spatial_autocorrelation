...
mult = 10 # simply for making the rain stronger
ts = 49#120#22#3*17 #timestamp
cod = 70 #cutoff distance (km)
opt = True
bandwidth = 1.0 # km
...
if identical_l == True: 
    l_dist = 'N'
dir_path = Path('/home/adameshel/Documents/code/autocorr/semi_real/main/')
...
rad_current = str(str(agg_times[0]) + '_ts' +\
                ...
                str(mult) + '_Dzeros' +\
                str(discard_zeros))
rad_path_parent = Path('/home/adameshel/Documents/code/autocorr/radar_autocorr_snaps/')
...
for il, l in enumerate(cml_lengths):
    ...
    with open(dir_path_current.joinpath('ac_par_il_' + length_name + '.pkl'), 'wb') as f:
